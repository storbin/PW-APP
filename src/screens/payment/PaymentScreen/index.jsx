...
export const PaymentScreen = ({ navigation }) => {
	...
	const { user_info, users_list, transactions_list, error } = useSelector((state) => state.transactions)
	...
	return isLoading ? (
		...
		<SafeAreaView style={styles.screen}>
			<View style={styles.container}>
				<Formik initialValues={initialValues} onSubmit={handleCreateTransaction}>
					{({ handleChange, handleSubmit, values }) => (
						<View style={styles.container}>
							...
							<FlatList
								...
								renderItem={({ item }) => {
									return (
										<TouchableOpacity style={styles.userTransactionsItem} onPress={() => handleTouchableOpacityPress({ handleChange }, item)}>
											<MyText color={colors.BLACK}>Name: {item.username}</MyText>
